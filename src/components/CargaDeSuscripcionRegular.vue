<template>
	<v-card class="pa-3">
		<v-card-title>
			<h5 class="headline">Modificar precio de suscripcion Regular</h5>
		</v-card-title>
		...
		<v-card-text>
			<v-form v-model="formularioEsValido" ref="formulario">
				...
				<v-text-field
					...
					label="Monto nuevo a cobrar para la suscripcion regular"
		...
		<v-card-actions>
			...
			<v-btn
				class="success"
				:loading="esperandoCreacionDeSuscripcionRegular"
				:disabled="!formularioEsValido"
				@click.stop="crearSuscripcionRegular( )"
			>
				Cargar
			</v-btn>
		</v-card-actions>
...
<script lang="ts">
	...
	export default class CargaDeSuscripcion extends Vue {
		 ...
		 */
		public esperandoCreacionDeSuscripcionRegular: boolean = false;
		 ...
		 */
		public async crearSuscripcionRegular( ): Promise<void> {
			this.esperandoCreacionDeSuscripcionRegular = true;
			await this.$store.dispatch( 'crearSuscripcion', this.modelo );
			this.esperandoCreacionDeSuscripcionRegular = false;
